...
class GameClient:
    ...
    def diff(self, board1, board2) -> list[tuple[int, int]]:
        """
            ...
            # Nhận thông tin trò chơi
                ...
                if data.get("turn") in self.team_id:
                    if self.ai.turn == 0:
                        ...
                        self.ai.turn *= -1
                        self.prvBoard = copy.deepcopy(data.get("board"))
                        self.ai.boardConvert(self.prvBoard)
                        game_info["board"] = self.prvBoard
                        self.send_move()
                    else:
                        # evaluate lai ban co
                        self.board = copy.deepcopy(data.get("board"))
                        prvmove = self.diff(board1=self.board, board2=self.prvBoard)
                        print("day la nuoc di moi:", prvmove)
                        if prvmove != []:
                            self.ai.boardValue = self.ai.evaluate(prvmove[0][0], prvmove[0][1], self.ai.boardValue, -1, self.ai.nextBound)
                            self.ai.updateBound(prvmove[0][0], prvmove[0][1], self.ai.nextBound)
                            self.ai.currentI, self.ai.currentJ = prvmove[0][0], prvmove[0][1]
                            # thực hiện nước đi và cập nhật bảng zobrist
                            self.ai.setState(prvmove[0][0], prvmove[0][1], self.ai.turn)
                            print("is that turn AI?", self.ai.turn)
                            self.ai.rollingHash ^= self.ai.zobristTable[prvmove[0][0]][prvmove[0][1]][1]
